...
| 控件 | 作用 |
| --- | --- |
| 绘制路径单选按钮 | 在传统路径与 indirect 路径之间切换 |
...
三个参数依次为实例数量、远裁剪面距离、每条路径的运行秒数。脚本关闭界面分别运行两条路径，抓取画面并打印两张 PNG 的 SHA256 校验值。
...
前者在一千、两万、二十万、一百万实例下分别运行两条路径，对比可见实例数量；后者在三组配置下测量两条路径的耗时。两个脚本都关闭界面运行。
...
| --- | --- | --- | --- | --- | --- |
| 传统 | 435 | 2.67 ms | 0.50 ms | 0.58 ms | 1.49 ms |
| indirect | 1 | 1.76 ms | 0.00 ms | 0.18 ms | 1.50 ms |

20 万实例，远裁剪面 420，可见 2905 个实例：

| 路径 | 绘制命令 | 帧时间 | 主机剔除 | 主机记录命令 | 设备时间 |
| --- | --- | --- | --- | --- | --- |
| 传统 | 2905 | 12.85 ms | 0.50 ms | 2.75 ms | 9.44 ms |
| indirect | 1 | 10.00 ms | 0.00 ms | 0.30 ms | 9.55 ms |

100 万实例，远裁剪面 160，可见 435 个实例：

| 路径 | 绘制命令 | 帧时间 | 主机剔除 | 主机记录命令 | 设备时间 |
| --- | --- | --- | --- | --- | --- |
| 传统 | 435 | 4.41 ms | 2.18 ms | 0.63 ms | 1.50 ms |
| indirect | 1 | 1.77 ms | 0.00 ms | 0.17 ms | 1.52 ms |
...
在一千、两万、二十万、一百万实例下，两条路径给出的可见实例数量分别为 269、666、666、666，逐项相同。

运行时反复切换路径并同时改变实例数量，可见实例数量始终与另一条路径吻合，验证层没有报告任何问题。
