...
表头中标明计时发生的位置：带 `CPU` 前缀的列是主机侧各阶段的耗时，剔除、命令缓冲录制与提交队列都在 CPU 上执行；`GPU 设备时间` 由时间戳查询测得，是这一帧的命令实际占用 GPU 的时长；`帧时间（端到端）` 是主循环相邻两次帧开始的间隔，等于整帧的花费，同时包含主机侧工作与等待 GPU 的时间。

20 万实例，远裁剪面 160，可见 435 个实例：

| 路径 | 绘制命令 | 帧时间（端到端） | CPU 主机剔除 | CPU 记录：起始 | CPU 记录：剔除调度 | CPU 记录：G-Buffer 通道 | CPU 记录：光照通道 | CPU 记录：提交命令 | GPU 设备时间 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
