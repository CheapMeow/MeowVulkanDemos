...
当你在 markdown 中写公式时，使用 markdown 的公式块语法，而不要使用代码块。
在写例如 README 这种 markdown 的时候，如果指示了要附带源码，那么一定要写成 path:line 这种超链接的形式，方便代码阅读。这种超链接的跳转是以 markdown 所在的目录为根目录做跳转的，所以超链接里面的源码路径要注意相对路径的写法。不能写成绝对路径，因为不
不允许在Bash命令里面inline超长的、超多行Bash命令或者是超长的Python脚本。如果你需要执行一个脚本，你要先写到文件里。

在写任何Python代码的时候，都不允许在文件的最前面添加docstring，也不允许添加shebang
代码注释使用中文，术语使用英文，stdout/stderr 输出使用英文；不要过度注释
所有的 commit 都要以 [AI] 开头，和后面的注释内容之间有一个空格，注释内容应该是英文
...
## 脱敏要求
**不要**修改我的环境变量
如果你使用了外部的依赖库，那么必须通过**环境变量**来获取外部的依赖的路径，然后在构建系统开放接口，允许通过脚本指定依赖路径，以便在我的环境变量明显不符合要求，但是我的环境变量你又不能修改，所以你可以写一个脚本来传递这个路径，以便覆盖环境变量的值
用于传递本机的依赖路径的脚本必须通过 gitignore 来忽略掉