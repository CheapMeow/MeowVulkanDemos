...
// 报告里 case 自己的前几列
const char* const REPORT_HEADER_COLUMNS = "mode,lut_size,exposure,draw_commands";
...
static void initializeRendererStack(AppState& state, android_app* app)
{
    ...
    // 相机固定在初始位置，安卓端没有键盘输入
    state.camera.position = glm::vec3(0.0f, 0.9f, 7.2f);
    state.camera.yaw = 0.0f;
    state.camera.pitch = -0.06f;
    state.camera.verticalFieldOfView = glm::radians(45.0f);
    ...
    LOG_I("android color lut renderer initialized");
}
...
static void drawOneFrame(AppState& state)
{
    ...
    fillGradeUniform(state.uiState, gradeUniform);

    FrameInput input = {};
    input.drawUserInterface = true;
    input.gradeMode = state.uiState.gradeMode;
    input.lutSize = state.uiState.lutSize;

    FrameStatistics statistics = {};
    const bool frameDrawn = drawFrame(state.ctx, state.renderer, state.frameCounter, input,
                                      sceneUniform, gradeUniform, statistics);
    if (!frameDrawn) {
        rebuildSwapchain(state);
        return;
    }
    ...
    if (currentTime - state.lastPrintSeconds >= 2.0) {
        ...
        LOG_I("frame %.1f FPS, mode %s, lut size %u", fps, gradeModeName(state.uiState.gradeMode),
              state.uiState.lutSize);
        state.lastPrintSeconds = currentTime;
    }
}

// TCP 控制命令：mode/lut-size/exposure/light/ambient 改配置，
...
static void installControlHandler(AppState& state)
{
    state.controlServer.onCommand = [&state](const std::string& command) -> std::string {
        ...
        if (verb == "mode") {
            std::string value;
            if (!(stream >> value) ||
                (value != "none" && value != "lut3d" && value != "curves" && value != "analytic")) {
                return "err: mode takes none, lut3d, curves or analytic";
            }
            state.uiState.gradeMode = value == "none"       ? GRADE_MODE_NONE
                                      : value == "lut3d"    ? GRADE_MODE_LUT3D
                                      : value == "curves"   ? GRADE_MODE_CURVES
                                                            : GRADE_MODE_ANALYTIC;
        ...
        if (verb == "lut-size") {
            ...
            state.uiState.lutSize = static_cast<uint32_t>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
...
{
    AppState state;
    state.uiState.cameraMoveSpeed = 6.0f;
    state.uiState.gradeMode = GRADE_MODE_ANALYTIC;
    state.uiState.lutSize = 32;
    state.uiState.exposure = 1.0f;
    state.uiState.lightIntensity = 0.85f;
    state.uiState.ambient = 0.10f;
