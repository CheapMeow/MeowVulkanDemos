...
{
    if (mode == GRADE_MODE_LUT3D) {
        return "lut3d";
    }
    if (mode == GRADE_MODE_CURVES) {
        return "curves";
    }
    if (mode == GRADE_MODE_ANALYTIC) {
        return "analytic";
    }
    return "none";
}

static void createSceneRenderPass(const VulkanContext& ctx, GradeRenderer& renderer)
{
...
static void createOutputRenderPass(const VulkanContext& ctx, GradeRenderer& renderer)
{
...
static void createSwapchainTargets(const VulkanContext& ctx, GradeRenderer& renderer)
{
...
static void destroySwapchainTargets(const VulkanContext& ctx, GradeRenderer& renderer)
{
...
static void createDescriptorLayouts(const VulkanContext& ctx, GradeRenderer& renderer)
{
    ...
    VkDescriptorSetLayoutBinding gradeBindings[4] = {};
    gradeBindings[0].binding = 0;
    gradeBindings[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    gradeBindings[0].descriptorCount = 1;
    gradeBindings[0].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
    for (int i = 1; i < 4; ++i) {
        gradeBindings[i].binding = static_cast<uint32_t>(i);
        gradeBindings[i].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        gradeBindings[i].descriptorCount = 1;
        gradeBindings[i].stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;
    }

    VkDescriptorSetLayoutCreateInfo gradeLayoutInfo = {};
    gradeLayoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    gradeLayoutInfo.bindingCount = 4;
    gradeLayoutInfo.pBindings = gradeBindings;
    VK_CHECK(vkCreateDescriptorSetLayout(ctx.device, &gradeLayoutInfo, nullptr,
                                         &renderer.gradeSetLayout));
...
// 三维查找表的视图在重建时单独写入，这里只写与交换链尺寸相关的几项
static void writeGradeDescriptors(const VulkanContext& ctx, GradeRenderer& renderer, uint32_t index)
{
    FrameResources& frame = renderer.frames[index];
    writeBufferDescriptor(ctx, frame.gradeSet, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                          frame.gradeBuffer);
    writeImageDescriptor(ctx, frame.gradeSet, 1, renderer.hdrScene.view, renderer.lutSampler);
    writeImageDescriptor(ctx, frame.gradeSet, 3, renderer.curveTexture.view, renderer.lutSampler);
}

static void createMeshBuffers(const VulkanContext& ctx, const std::vector<ColorVertex>& vertices,
                              const std::vector<uint32_t>& indices, GpuBuffer& vertexBuffer,
                              GpuBuffer& indexBuffer, uint32_t& indexCount)
{
    indexCount = static_cast<uint32_t>(indices.size());

    const VkDeviceSize vertexBytes = sizeof(ColorVertex) * vertices.size();
...
// 三维查找表按尺寸重建，一维曲线与尺寸无关，只在创建时建一次
static void rebuildLut(const VulkanContext& ctx, GradeRenderer& renderer, uint32_t size)
{
    VK_CHECK(vkDeviceWaitIdle(ctx.device));

    if (renderer.lutTexture.image != VK_NULL_HANDLE) {
        destroyTexture(ctx, renderer.lutTexture);
    }

    const std::vector<unsigned char> pixels = buildThreeDimensionalLut(size);
    createTextureFromRgba(ctx, size * size, size, pixels.data(), false, renderer.lutTexture, nullptr);

    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        writeImageDescriptor(ctx, renderer.frames[i].gradeSet, 2, renderer.lutTexture.view,
                             renderer.lutSampler);
    }
    renderer.lutSize = size;
}
...
static void createScenePipelines(const VulkanContext& ctx, GradeRenderer& renderer)
{
    ...
    VkShaderModule patchVertexModule =
        loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/patch.vert.spv"));
    VkShaderModule patchFragmentModule =
        loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/patch.frag.spv"));
    VkShaderModule fullscreenVertexModule =
        loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/fullscreen.vert.spv"));

    VkVertexInputBindingDescription vertexBinding = {};
    vertexBinding.binding = 0;
    vertexBinding.stride = sizeof(ColorVertex);
    vertexBinding.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

    VkVertexInputAttributeDescription vertexAttributes[3] = {};
    vertexAttributes[0].location = 0;
    vertexAttributes[0].format = VK_FORMAT_R32G32B32_SFLOAT;
    vertexAttributes[0].offset = static_cast<uint32_t>(offsetof(ColorVertex, position));
    vertexAttributes[1].location = 1;
    vertexAttributes[1].format = VK_FORMAT_R32G32B32_SFLOAT;
    vertexAttributes[1].offset = static_cast<uint32_t>(offsetof(ColorVertex, normal));
    vertexAttributes[2].location = 2;
    vertexAttributes[2].format = VK_FORMAT_R32G32B32_SFLOAT;
    vertexAttributes[2].offset = static_cast<uint32_t>(offsetof(ColorVertex, color));
    ...
    // 参考块的顶点着色器不读法线，顶点输入里去掉那一项
    VkVertexInputAttributeDescription patchAttributes[2] = { vertexAttributes[0], vertexAttributes[2] };
    VkPipelineVertexInputStateCreateInfo patchVertexInput = vertexInput;
    patchVertexInput.vertexAttributeDescriptionCount = 2;
    patchVertexInput.pVertexAttributeDescriptions = patchAttributes;
    ...
    sphereStages[0] = makeShaderStage(VK_SHADER_STAGE_VERTEX_BIT, sceneVertexModule);
    sphereStages[1] = makeShaderStage(VK_SHADER_STAGE_FRAGMENT_BIT, sceneFragmentModule);
    ...
    VkPipelineShaderStageCreateInfo patchStages[2] = {};
    patchStages[0] = makeShaderStage(VK_SHADER_STAGE_VERTEX_BIT, patchVertexModule);
    patchStages[1] = makeShaderStage(VK_SHADER_STAGE_FRAGMENT_BIT, patchFragmentModule);
    pipelineInfo.pStages = patchStages;
    pipelineInfo.pVertexInputState = &patchVertexInput;
    pipelineInfo.pDepthStencilState = &noDepth;
    rasterization.cullMode = VK_CULL_MODE_NONE;
    VK_CHECK(vkCreateGraphicsPipelines(ctx.device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr,
                                       &renderer.patchPipeline));

    vkDestroyShaderModule(ctx.device, fullscreenVertexModule, nullptr);
    vkDestroyShaderModule(ctx.device, patchFragmentModule, nullptr);
    vkDestroyShaderModule(ctx.device, patchVertexModule, nullptr);
    ...
    vkDestroyShaderModule(ctx.device, sceneVertexModule, nullptr);
}
...
void createRenderer(const VulkanContext& ctx, GradeRenderer& renderer)
{
    ...
    std::vector<ColorVertex> sphereVertices;
    std::vector<uint32_t> sphereIndices;
    buildSphereRow(sphereVertices, sphereIndices);
    createMeshBuffers(ctx, sphereVertices, sphereIndices, renderer.sphereVertexBuffer,
                      renderer.sphereIndexBuffer, renderer.sphereIndexCount);

    std::vector<ColorVertex> patchVertices;
    std::vector<uint32_t> patchIndices;
    buildReferencePatches(patchVertices, patchIndices);
    createMeshBuffers(ctx, patchVertices, patchIndices, renderer.patchVertexBuffer,
                      renderer.patchIndexBuffer, renderer.patchIndexCount);

    const std::vector<unsigned char> curvePixels = buildOneDimensionalLut();
    createTextureFromRgba(ctx, GRADE_ONE_DIMENSIONAL_SIZE, 1, curvePixels.data(), false,
                          renderer.curveTexture, nullptr);

    // 查找表只读最细的一级：屏幕空间导数大的地方（比如色块边缘）会被吸到粗层上，
    // 插值出来的颜色就跟着糊掉，所以这里把最大层级钉在零
    ...
    VK_CHECK(vkCreateSampler(ctx.device, &samplerInfo, nullptr, &renderer.lutSampler));
    ...
    layoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    layoutInfo.setLayoutCount = 1;
    layoutInfo.pSetLayouts = &renderer.gradeSetLayout;
    VK_CHECK(vkCreatePipelineLayout(ctx.device, &layoutInfo, nullptr, &renderer.gradePipelineLayout));

    renderer.gradePipeline = createFullscreenPipeline(ctx, renderer.gradePipelineLayout,
                                                      renderer.outputRenderPass,
                                                      DEMO_SHADER_DIR "/grade.frag.spv");
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        ...
        createBuffer(ctx, sizeof(SceneUniform), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     ...
                     frame.sceneBuffer);
        createBuffer(ctx, sizeof(GradeUniform), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     frame.gradeBuffer);
        ...
        frame.gradeSet = allocateDescriptorSet(ctx, renderer.descriptorPool, renderer.gradeSetLayout);
        writeGradeDescriptors(ctx, renderer, static_cast<uint32_t>(i));
...
void destroyRenderer(const VulkanContext& ctx, GradeRenderer& renderer)
{
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        ...
        destroyBuffer(ctx, frame.gradeBuffer);
    ...
    vkDestroyPipeline(ctx.device, renderer.gradePipeline, nullptr);
    vkDestroyPipelineLayout(ctx.device, renderer.gradePipelineLayout, nullptr);
    vkDestroyPipeline(ctx.device, renderer.patchPipeline, nullptr);
    ...
    vkDestroyDescriptorSetLayout(ctx.device, renderer.gradeSetLayout, nullptr);
    ...
    destroyTexture(ctx, renderer.lutTexture);
    destroyTexture(ctx, renderer.curveTexture);
    vkDestroySampler(ctx.device, renderer.lutSampler, nullptr);

    destroyBuffer(ctx, renderer.patchIndexBuffer);
    destroyBuffer(ctx, renderer.patchVertexBuffer);
               ...
               const GradeUniform& gradeUniform, FrameStatistics& outStatistics)
{
    ...
    // 表格尺寸变化时重建三维查找表
    if (input.lutSize != renderer.lutSize) {
        rebuildLut(ctx, renderer, input.lutSize);
    }

    outStatistics.gpuMilliseconds = 0.0;
    outStatistics.gpuSceneMilliseconds = 0.0;
    outStatistics.gpuGradeMilliseconds = 0.0;
    if (renderer.timestampsSupported && frame.timestampsValid) {
        ...
        outStatistics.gpuGradeMilliseconds =
    ...
    std::memcpy(frame.gradeBuffer.mapped, &gradeUniform, sizeof(GradeUniform));
    ...
    vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer.patchPipeline);
    vkCmdBindVertexBuffers(frame.commandBuffer, 0, 1, &renderer.patchVertexBuffer.buffer, &vertexOffset);
    vkCmdBindIndexBuffer(frame.commandBuffer, renderer.patchIndexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);
    vkCmdDrawIndexed(frame.commandBuffer, renderer.patchIndexCount, 1, 0, 0, 0);
    ++drawCallCount;
    ...
    const double gradeStart = nowSeconds();
    ...
    vkCmdSetScissor(frame.commandBuffer, 0, 1, &scissor);
    vkCmdBindDescriptorSets(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            renderer.gradePipelineLayout, 0, 1, &frame.gradeSet, 0, nullptr);
    vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer.gradePipeline);
    vkCmdDraw(frame.commandBuffer, 3, 1, 0, 0);
    ++drawCallCount;
    ...
    vkCmdEndRenderPass(frame.commandBuffer);
    ++renderPassCount;
    outStatistics.cpuRecordGradeMilliseconds = (nowSeconds() - gradeStart) * 1000.0;
