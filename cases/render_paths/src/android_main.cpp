...
static void initializeRendererStack(AppState& state, android_app* app)
{
    ...
    LOG_I("android render paths renderer initialized");
}
...
static void drawOneFrame(AppState& state)
{
    ...
    state.sceneTime += deltaSeconds;
    updateLights(state.renderer, state.uiState.lightCount, state.sceneTime);
    ...
    input.options.lightCount = state.uiState.lightCount;
    input.options.tileSize = state.uiState.tileSize;
    input.options.maxLightsPerTile = state.uiState.maxLightsPerTile;
    ...
    state.uiStatistics.tileCount = statistics.tileCount;
    ...
    if (currentTime - state.lastPrintSeconds >= 2.0) {
        ...
        LOG_I("frame %.1f FPS, path %s, lights %u", fps, renderPathName(state.uiState.path),
              state.uiState.lightCount);
...
static void installControlHandler(AppState& state)
{
    state.controlServer.onCommand = [&state](const std::string& command) -> std::string {
        ...
        if (verb == "lights") {
            ...
            state.uiState.lightCount = static_cast<uint32_t>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "tile-size") {
            ...
            state.uiState.tileSize = static_cast<uint32_t>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "max-per-tile") {
            ...
            state.uiState.maxLightsPerTile = static_cast<uint32_t>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "time") {
            ...
            state.sceneTime = static_cast<float>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "begin") {
...
{
    AppState state;
    state.uiState.path = RENDER_PATH_FORWARD;
    state.uiState.lightCount = 64;
    state.uiState.tileSize = 32;
    state.uiState.maxLightsPerTile = 32;
