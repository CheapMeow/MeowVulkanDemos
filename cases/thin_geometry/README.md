...
| 抗锯齿方式 | 做法 | 覆盖信息的来源 |
| --- | --- | --- |
| 无抗锯齿 | 单采样光栅化 | 只有像素中心是否落在图元内 |
| 多重采样 4x | 采样点参与覆盖判定，解析后得到灰度 | 每个像素四个采样点 |
| FXAA 后处理 | 单采样光栅化之后按亮度差找边缘再平滑 | 只有已经光栅化完的最终图像 |
...
几何通道的目标采样数随方式变化，多重采样时先解析到单采样贴图；合成通道用同一个全屏三角形，按方式选择
拷贝着色器或 FXAA 着色器，界面也记录在这个通道里。

## 实现要点

### 细杆宽度小于一个像素时的可见性

细杆宽度取 0.4 像素，平移偏移取零，抓帧后取第 400 行第 160 到 259 列，亮度明显高于背景的位置如下
（背景亮度 13，细杆亮度 255）：
...
无抗锯齿时一根 0.4 像素宽的细杆只有在刚好盖住某个像素中心时才会出现，因此这一段里 64 根细杆只看得见
一根，而且一亮就是满亮 255。多重采样 4x 按采样点判定覆盖，同样这一段里看得见三根，亮度是 73，也就是
四分之一的覆盖。FXAA 拿到的图像与无抗锯齿完全相同，它只能把已经出现的那一根从 255 压到 192，没有
办法让另外两根出现。
...
锁频 2880/15001 之后按段测量，细杆宽度 0.4 像素，各配置的绘制命令条数都是 2（几何一次实例化绘制，
合成一次全屏绘制）：
...
多重采样 4x 比无抗锯齿多出解析与四倍采样的覆盖测试，从 16 根到 256 根时设备时间从 0.020 涨到
0.030 毫秒，随细杆数量上升。FXAA 的后处理本身开销与细杆数量无关，从 16 根到 256 根时涨到 0.021
毫秒的那部分来自几何通道本身。本 case 的画面很轻，绝对时间只有几十微秒，标准差与差值同量级，只有
多重采样相对无抗锯齿的那一档差别稳定可辨。
