...
static void initializeRendererStack(AppState& state, android_app* app)
{
    ...
    LOG_I("android transparency renderer initialized, layers %u", state.uiState.layerCount);
}
...
static void drawOneFrame(AppState& state)
{
    ...
    input.options.layerCount = state.uiState.layerCount;
    input.options.heatmap = state.uiState.heatmap;
    ...
    state.uiStatistics.nodeCount = statistics.nodeCount;
    state.uiStatistics.nodeOverflow = statistics.nodeOverflow;
    state.uiStatistics.listBytes = statistics.listBytes;
    if (state.uiState.mode != TRANSPARENCY_LINKED_LIST) {
        state.uiStatistics.nodeCount = 0;
        state.uiStatistics.nodeOverflow = 0;
    }
    ...
    if (currentTime - state.lastPrintSeconds >= 2.0) {
        ...
        LOG_I("frame %.1f FPS, mode %s, layers %u, nodes %u", fps,
              transparencyModeName(state.uiState.mode), state.uiState.layerCount,
              state.uiStatistics.nodeCount);
...
static void installControlHandler(AppState& state)
{
    state.controlServer.onCommand = [&state](const std::string& command) -> std::string {
        ...
        if (verb == "layers") {
            ...
            state.uiState.layerCount = static_cast<uint32_t>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "heatmap") {
...
{
    AppState state;
    state.uiState.mode = TRANSPARENCY_FAR_TO_NEAR;
    state.uiState.layerCount = 8;
    state.uiState.heatmap = false;
