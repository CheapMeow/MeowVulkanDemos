...
static const char* const REPORT_HEADER_COLUMNS =
    "scene,samples,resolve,alpha_to_coverage,fragment_count,draw_commands";

static std::string reportPrefix(const UiState& state, const UiStatistics& statistics)
{
    char prefix[192];
    std::snprintf(prefix, sizeof(prefix), "%u,%u,%s,%d,%u,%u", state.sceneIndex, state.sampleCount,
                  msaaResolveModeName(state.resolveMode), state.alphaToCoverage ? 1 : 0,
                  statistics.fragmentCount, statistics.drawCallCount);
    return std::string(prefix);
}

static void rebuildSwapchainResources(VulkanContext& ctx, MsaaRenderer& renderer, GpuBuffer& captureBuffer,
                                      bool capturePending)
{
...
static void fillSceneUniform(const VulkanContext& ctx, const UiState& state, MsaaSceneUniform& outUniform)
{
    ...
    outUniform.modeParams = glm::vec4(static_cast<float>(state.sceneIndex),
                                      state.alphaToCoverage ? 1.0f : 0.0f, state.fragmentCost, 0.0f);
}
...
int main(int argc, char** argv)
{
    ...
    uint32_t resolveMode = RESOLVE_HARDWARE;
    uint32_t sceneIndex = 0;
    bool alphaToCoverage = true;
    float fragmentCost = 32.0f;
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--samples") == 0 && i + 1 < argc) {
            ...
            if (std::strcmp(value, "hardware") == 0) {
                resolveMode = RESOLVE_HARDWARE;
            } else if (std::strcmp(value, "custom") == 0) {
                resolveMode = RESOLVE_CUSTOM;
            } else {
                FATAL("--resolve takes hardware or custom");
            }
            ++i;
        } else if (std::strcmp(argv[i], "--scene") == 0 && i + 1 < argc) {
            sceneIndex = static_cast<uint32_t>(std::atoi(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--no-coverage") == 0) {
            alphaToCoverage = false;
        } else if (std::strcmp(argv[i], "--fragment-cost") == 0 && i + 1 < argc) {
            fragmentCost = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
    ...
    GLFWwindow* window = glfwCreateWindow(1600, 900, "Vulkan MSAA Demo", nullptr, nullptr);
    if (window == nullptr) {
    ...
    MsaaRenderer renderer = {};
    createRenderer(ctx, renderer, sampleCount, resolveMode);
    ...
    UiState uiState = {};
    uiState.sceneIndex = sceneIndex;
    uiState.sampleCount = sampleCount;
    uiState.resolveMode = resolveMode;
    uiState.alphaToCoverage = alphaToCoverage;
    uiState.fragmentCost = fragmentCost;
    ...
    controlServer.onCommand = [&](const std::string& command) -> std::string {
        ...
        if (verb == "resolve") {
            std::string value;
            if (!(stream >> value) || (value != "hardware" && value != "custom")) {
                return "err: resolve takes hardware or custom";
            }
            uiState.resolveMode = value == "custom" ? RESOLVE_CUSTOM : RESOLVE_HARDWARE;
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "scene") {
            long value = 0;
            if (!(stream >> value) || value < 0 || value > 2) {
                return "err: scene takes 0, 1 or 2";
            }
            uiState.sceneIndex = static_cast<uint32_t>(value);
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "coverage") {
            ...
            if (!(stream >> value) || (value != 0 && value != 1)) {
                return "err: coverage takes 0 or 1";
            }
            uiState.alphaToCoverage = value == 1;
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "fragment-cost") {
            ...
            uiState.fragmentCost = static_cast<float>(value);
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "background") {
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        input.options.resolveMode = uiState.resolveMode;
        input.options.alphaToCoverage = uiState.alphaToCoverage;
        input.options.sceneIndex = uiState.sceneIndex;
        input.options.fragmentCost = uiState.fragmentCost;
        input.options.backgroundIntensity = uiState.backgroundIntensity;
        ...
        if (currentTime - lastPrintTime >= 0.5) {
            std::printf("samples %u | resolve %s | scene %u | fragment calls %u | draw commands %u\n",
                        uiState.sampleCount, msaaResolveModeName(uiState.resolveMode), uiState.sceneIndex,
