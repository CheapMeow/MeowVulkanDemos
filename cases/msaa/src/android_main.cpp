...
// 报告里 case 自己的前几列
const char* const REPORT_HEADER_COLUMNS = "scene,samples,resolve,alpha_to_coverage,fragment_count,draw_commands";
...
static void initializeRendererStack(AppState& state, android_app* app)
{
    ...
    createRenderer(state.ctx, state.renderer, state.uiState.sampleCount, state.uiState.resolveMode);
    ...
    LOG_I("android msaa renderer initialized, samples %u", state.uiState.sampleCount);
}
...
static void drawOneFrame(AppState& state)
{
    ...
    input.options.resolveMode = state.uiState.resolveMode;
    input.options.alphaToCoverage = state.uiState.alphaToCoverage;
    input.options.sceneIndex = state.uiState.sceneIndex;
    input.options.fragmentCost = state.uiState.fragmentCost;
    input.options.backgroundIntensity = state.uiState.backgroundIntensity;
    ...
    if (currentTime - state.lastPrintSeconds >= 2.0) {
        ...
        LOG_I("frame %.1f FPS, samples %u, resolve %s, fragment calls %u", fps, state.uiState.sampleCount,
              msaaResolveModeName(state.uiState.resolveMode), statistics.fragmentCount);
...
static void installControlHandler(AppState& state)
{
    state.controlServer.onCommand = [&state](const std::string& command) -> std::string {
        ...
        if (verb == "resolve") {
            ...
            state.uiState.resolveMode = value == "custom" ? RESOLVE_CUSTOM : RESOLVE_HARDWARE;
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "scene") {
            ...
            state.uiState.sceneIndex = static_cast<uint32_t>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "coverage") {
            ...
            state.uiState.alphaToCoverage = value == 1;
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "fragment-cost" || verb == "background") {
            double value = 0.0;
            if (!(stream >> value) || value < 0.0) {
                return "err: " + verb + " out of range";
            }
            if (verb == "fragment-cost") {
                state.uiState.fragmentCost = static_cast<float>(value);
            } else {
...
{
    AppState state;
    state.uiState.sceneIndex = 0;
    state.uiState.sampleCount = 4;
    state.uiState.resolveMode = RESOLVE_HARDWARE;
    state.uiState.alphaToCoverage = true;
    state.uiState.fragmentCost = 32.0f;
    state.uiState.backgroundIntensity = 2.0f;
