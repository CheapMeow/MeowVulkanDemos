...
{
    char prefix[192];
    std::snprintf(prefix, sizeof(prefix), "%s,%.3f,%.1f,%.2f,%.0f,%u,%u,%u",
                  dofModeName(state.mode), state.focalLength, state.fNumber, state.focusDistance,
                  state.maxRadius, state.sampleCount, state.blades, drawCommands);
    return std::string(prefix);
}

// 交换链以及所有跟它的尺寸、图像数量绑定的资源都要在窗口尺寸变化后重建
static void rebuildSwapchainResources(VulkanContext& ctx, DofRenderer& renderer, GpuBuffer& captureBuffer,
                                      bool capturePending)
{
...
int main(int argc, char** argv)
{
    ...
    UiState uiState = {};
    uiState.cameraMoveSpeed = 1.0f;
    uiState.mode = DOF_MODE_DISK;
    uiState.focalLength = 0.05f;
    uiState.fNumber = 1.4f;
    uiState.focusDistance = 2.9f;
    uiState.maxRadius = 32.0f;
    uiState.sampleCount = 128;
    uiState.jitter = 0.5f;
    uiState.blades = 0;
    uiState.exposure = 1.0f;
    uiState.lightIntensity = 0.9f;
    uiState.ambient = 0.08f;
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--auto-exit") == 0 && i + 1 < argc) {
            ...
            if (!parseMode(argv[i + 1], uiState.mode)) {
                FATAL("--mode takes off, gaussian or disk");
            }
            ++i;
        } else if (std::strcmp(argv[i], "--focal") == 0 && i + 1 < argc) {
            uiState.focalLength = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--fnumber") == 0 && i + 1 < argc) {
            uiState.fNumber = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--focus") == 0 && i + 1 < argc) {
            uiState.focusDistance = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--max-radius") == 0 && i + 1 < argc) {
            uiState.maxRadius = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--samples") == 0 && i + 1 < argc) {
            uiState.sampleCount = static_cast<uint32_t>(std::atoi(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--jitter") == 0 && i + 1 < argc) {
            uiState.jitter = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--blades") == 0 && i + 1 < argc) {
            uiState.blades = static_cast<uint32_t>(std::atoi(argv[i + 1]));
            ++i;
    ...
    GLFWwindow* window = glfwCreateWindow(1600, 900, "Vulkan Depth of Field Demo", nullptr, nullptr);
    if (window == nullptr) {
    ...
    Camera camera = {};
    camera.position = glm::vec3(0.0f, 0.35f, 0.5f);
    camera.yaw = 0.0f;
    camera.pitch = -0.10f;
    ...
    controlServer.onCommand = [&](const std::string& command) -> std::string {
        ...
        if (verb == "blades") {
            ...
            uiState.blades = static_cast<uint32_t>(value);
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "focal" || verb == "fnumber" || verb == "focus" || verb == "max-radius" ||
            ...
            if (verb == "focal") {
                uiState.focalLength = value;
            } else if (verb == "fnumber") {
                uiState.fNumber = value;
            } else if (verb == "focus") {
                uiState.focusDistance = value;
            } else if (verb == "max-radius") {
                uiState.maxRadius = value;
            } else if (verb == "jitter") {
                uiState.jitter = value;
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        fillDofUniform(ctx, uiState, dofUniform);
        ...
        const bool frameDrawn =
            drawFrame(ctx, renderer, frameCounter, input, sceneUniform, dofUniform, statistics);
        if (!frameDrawn) {
            rebuildSwapchainResources(ctx, renderer, captureBuffer, captureRequested && !captureDone);
            continue;
        }
        ...
        if (currentTime - lastPrintTime >= 0.5) {
            std::printf("mode %s | focal %.3f | f/%.1f | focus %.2f | radius %.0f | samples %u | "
                        "draw commands %u\n",
                        dofModeName(uiState.mode), uiState.focalLength, uiState.fNumber,
                        uiState.focusDistance, uiState.maxRadius, uiState.sampleCount,
