...
static void fillUniform(const VulkanContext& ctx, const UiState& state, SsaoSceneUniform& outUniform)
{
    ...
    outUniform.miscParams =
        glm::vec4(state.normalWeighting ? 1.0f : 0.0f, state.strength, kNearPlane, kFarPlane);
}
...
static void initializeRendererStack(AppState& state, android_app* app)
{
    ...
    LOG_I("android ssao renderer initialized");
}
...
static void drawOneFrame(AppState& state)
{
    ...
    input.options.radius = state.uiState.radius;
    input.options.kernelSize = state.uiState.kernelSize;
    input.options.normalWeighting = state.uiState.normalWeighting;
    input.options.strength = state.uiState.strength;
    ...
    if (currentTime - state.lastPrintSeconds >= 2.0) {
        ...
        LOG_I("frame %.1f FPS, mode %s, samples %u", fps, ssaoModeName(state.uiState.mode),
              state.uiState.samples);
...
static void installControlHandler(AppState& state)
{
    state.controlServer.onCommand = [&state](const std::string& command) -> std::string {
        ...
        if (verb == "radius" || verb == "kernel" || verb == "strength") {
            ...
            if (verb == "radius") {
                state.uiState.radius = static_cast<float>(value);
            } else if (verb == "kernel") {
                state.uiState.kernelSize = static_cast<float>(value);
            } else {
                state.uiState.strength = static_cast<float>(value);
            }
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "normal-weight") {
            ...
            state.uiState.normalWeighting = value == 1;
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "begin") {
...
{
    AppState state;
    state.uiState.mode = SSAO_MODE_AMBIENT;
    state.uiState.samples = 16;
    state.uiState.radius = 0.5f;
    state.uiState.kernelSize = 1.0f;
    state.uiState.strength = 1.5f;
    state.uiState.normalWeighting = true;
