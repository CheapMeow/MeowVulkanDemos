...
{
    if (kind == NOISE_PERLIN) {
        return "perlin";
    }
    if (kind == NOISE_WORLEY) {
        return "worley";
    }
    if (kind == NOISE_FBM) {
        return "fbm";
    }
    if (kind == NOISE_REFERENCE) {
        return "reference";
    }
    return "value";
}
...
{
    if (kind == NOISE_REFERENCE) {
        return referenceSide * referenceSide * octaves;
    }
    if (kind == NOISE_FBM) {
        return octaves;
    }
...
static void createRenderPass(const VulkanContext& ctx, NoiseRenderer& renderer)
{
...
static void createSwapchainTargets(const VulkanContext& ctx, NoiseRenderer& renderer)
{
...
static void destroySwapchainTargets(const VulkanContext& ctx, NoiseRenderer& renderer)
{
...
static void createDescriptorLayout(const VulkanContext& ctx, NoiseRenderer& renderer)
{
...
static void createPipeline(const VulkanContext& ctx, NoiseRenderer& renderer)
{
    VkPipelineLayoutCreateInfo layoutInfo = {};
    layoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    layoutInfo.setLayoutCount = 1;
    layoutInfo.pSetLayouts = &renderer.noiseSetLayout;
    ...
    VkShaderModule fragmentModule =
        loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/noise.frag.spv"));
...
void createRenderer(const VulkanContext& ctx, NoiseRenderer& renderer)
{
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        ...
        createBuffer(ctx, sizeof(NoiseUniform), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     frame.uniformBuffer);

        frame.noiseSet = allocateDescriptorSet(ctx, renderer.descriptorPool, renderer.noiseSetLayout);
        writeBufferDescriptor(ctx, frame.noiseSet, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                              frame.uniformBuffer);
...
void destroyRenderer(const VulkanContext& ctx, NoiseRenderer& renderer)
{
    ...
    vkDestroyPipeline(ctx.device, renderer.noisePipeline, nullptr);
    ...
    vkDestroyDescriptorSetLayout(ctx.device, renderer.noiseSetLayout, nullptr);
               ...
               const FrameInput& input, const NoiseUniform& uniform, FrameStatistics& outStatistics)
{
    ...
    std::memcpy(frame.uniformBuffer.mapped, &uniform, sizeof(NoiseUniform));
    ...
    const double noiseStart = nowSeconds();
    ...
    vkCmdBindDescriptorSets(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer.pipelineLayout,
                            0, 1, &frame.noiseSet, 0, nullptr);
    vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer.noisePipeline);
    vkCmdDraw(frame.commandBuffer, 3, 1, 0, 0);

    outStatistics.drawCallCount = 1;
    outStatistics.renderPassCount = 1;
    outStatistics.cpuRecordNoiseMilliseconds = (nowSeconds() - noiseStart) * 1000.0;
