...
{
    return valueMode == SHADOW_VALUE_VSM ? VK_FORMAT_R16G16_SFLOAT : VK_FORMAT_R32_SFLOAT;
}

const char* shadowValueModeName(uint32_t valueMode)
{
    return valueMode == SHADOW_VALUE_VSM ? "vsm" : "depth";
}

static void createShadowSampler(const VulkanContext& ctx, ShadowRenderer& renderer)
{
    // 深度值或矩保存在颜色附件里，采样器不做硬件比较，逐纹素取值后在着色器里处理。
    // 滤波必须是最近邻，否则会把相邻纹素的值平均掉
...
static void createShadowRenderPass(const VulkanContext& ctx, ShadowRenderer& renderer)
{
    ...
    // 保存深度值或矩的颜色附件，主通道以采样方式读取
    attachments[0].format = shadowMapColorFormat(renderer.valueMode);
...
static void createShadowTargets(const VulkanContext& ctx, ShadowRenderer& renderer)
{
    ...
    for (uint32_t cascade = 0; cascade < MAX_CASCADE_COUNT; ++cascade) {
        createAttachmentTexture(ctx, size, size, shadowMapColorFormat(renderer.valueMode),
                                VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                                VK_IMAGE_ASPECT_COLOR_BIT, renderer.shadowMaps[cascade]);

        VkImageView views[2] = { renderer.shadowMaps[cascade].view, renderer.shadowDepth.view };
        ...
        VK_CHECK(vkCreateFramebuffer(ctx.device, &framebufferInfo, nullptr,
                                     &renderer.shadowFramebuffers[cascade]));
    }

    // 创建时先转到只读布局：关闭阴影时不跑阴影通道，主通道仍然绑定着这些贴图，
    // 布局必须与描述符里声明的只读布局一致
    ...
    for (uint32_t cascade = 0; cascade < MAX_CASCADE_COUNT; ++cascade) {
        ...
        barrier.image = renderer.shadowMaps[cascade].image;
...
{
    for (uint32_t cascade = 0; cascade < MAX_CASCADE_COUNT; ++cascade) {
        vkDestroyFramebuffer(ctx.device, renderer.shadowFramebuffers[cascade], nullptr);
        renderer.shadowFramebuffers[cascade] = VK_NULL_HANDLE;
        destroyTexture(ctx, renderer.shadowMaps[cascade]);
    }
...
static void createDescriptorLayouts(const VulkanContext& ctx, ShadowRenderer& renderer)
{
    ...
    // 0 到 4 是材质贴图，5 是一个长度为四的数组，四个元素依次对应四级阴影贴图
    ...
    materialBindings[5].descriptorCount = MAX_CASCADE_COUNT;
...
static void createShadowPipeline(const VulkanContext& ctx, ShadowRenderer& renderer)
{
    ...
    VkShaderModule vertexModule =
        loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/shadow.vert.spv"));
    // 深度模式与矩模式写入的内容不同，用两个片元着色器
    const std::string fragmentPath =
        std::string(DEMO_SHADER_DIR) +
        (renderer.valueMode == SHADOW_VALUE_VSM ? "/shadow_moments.frag.spv" : "/shadow_depth.frag.spv");
    VkShaderModule fragmentModule = loadShaderModuleFromMemory(ctx, readAssetBytes(fragmentPath.c_str()));
    ...
    // 颜色附件保存深度或矩，写入 R 通道或 RG 通道
    VkPipelineColorBlendAttachmentState blendAttachment = {};
    blendAttachment.colorWriteMask = renderer.valueMode == SHADOW_VALUE_VSM
    ...
    for (int pass = 0; pass < 2; ++pass) {
        ...
        rasterization.cullMode =
            groundPass ? VK_CULL_MODE_NONE
                       : (renderer.shadowBackFaceDepth ? VK_CULL_MODE_FRONT_BIT : VK_CULL_MODE_BACK_BIT);
        rasterization.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;
        rasterization.lineWidth = 1.0f;
        // 贴图里的值由片元着色器写进颜色附件，光栅化的深度偏移只作用在深度测试上
        ...
        VkPipeline* targetPipeline = groundPass ? &renderer.shadowGroundPipeline : &renderer.shadowPipeline;
...
// 四级阴影贴图作为一组写进绑定 5 的数组，元素顺序与着色器里的下标一致
static void writeCascadeDescriptors(const VulkanContext& ctx, ShadowRenderer& renderer)
{
    VkDescriptorImageInfo imageInfos[MAX_CASCADE_COUNT] = {};
    for (uint32_t cascade = 0; cascade < MAX_CASCADE_COUNT; ++cascade) {
        imageInfos[cascade].sampler = renderer.shadowSampler;
        imageInfos[cascade].imageView = renderer.shadowMaps[cascade].view;
        imageInfos[cascade].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    }

    VkWriteDescriptorSet write = {};
    write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    write.dstSet = renderer.materialSet;
    write.dstBinding = 5;
    write.descriptorCount = MAX_CASCADE_COUNT;
                    ...
                    uint32_t groundInstanceIndex, const ShadowOptions& shadowOptions)
{
    ...
    renderer.cascadeCount = shadowOptions.cascadeCount;
    renderer.valueMode = shadowOptions.valueMode;
    renderer.shadowBackFaceDepth = true;
    ...
    writeCascadeDescriptors(ctx, renderer);
...
// 尺寸、级数或取值方式改变时重建阴影通道的全部资源：渲染通道与管线的附件格式跟随取值方式，
// 帧缓冲与纹理跟随尺寸与级数，最后把新的贴图视图写回材质描述符
static void applyShadowOptions(const VulkanContext& ctx, ShadowRenderer& renderer, const ShadowOptions& options)
{
    const bool targetsChanged = renderer.shadowMapSize != options.mapSize ||
                                renderer.cascadeCount != options.cascadeCount ||
                                renderer.valueMode != options.valueMode;
    if (!targetsChanged) {
        return;
    }
               ...
               FrameStatistics& outStatistics)
{
    ...
    if (input.shadowsEnabled) {
        ...
        // 颜色附件的初值：深度模式取最远深度，矩模式的一阶矩取最远深度、二阶矩取它的平方
        VkClearValue shadowClearValues[2] = {};
        shadowClearValues[0].color.float32[0] = 1.0f;
        shadowClearValues[0].color.float32[1] = 1.0f;
        ...
        for (uint32_t cascade = 0; cascade < renderer.cascadeCount; ++cascade) {
            ...
            shadowBegin.framebuffer = renderer.shadowFramebuffers[cascade];
            ...
            vkCmdBindDescriptorSets(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                    renderer.shadowPipelineLayout, 0, 1, &frame.sceneSet, 0, nullptr);

            const int32_t cascadeIndex = static_cast<int32_t>(cascade);
            vkCmdPushConstants(frame.commandBuffer, renderer.shadowPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0,
                               sizeof(cascadeIndex), &cascadeIndex);
            ...
            if (input.shadow.groundCaster) {
                ...
                vkCmdBindVertexBuffers(frame.commandBuffer, 0, 1, &renderer.groundVertexBuffer.buffer,
                                       &vertexOffset);
                vkCmdBindIndexBuffer(frame.commandBuffer, renderer.groundIndexBuffer.buffer, 0,
