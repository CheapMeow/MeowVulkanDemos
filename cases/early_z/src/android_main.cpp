...
// 报告里 case 自己的前几列
const char* const REPORT_HEADER_COLUMNS = "order,prepass,discard,alpha_test,fragment_cost,draw_commands";
...
static void initializeRendererStack(AppState& state, android_app* app)
{
    ...
    buildOverlappingQuadInstances(INSTANCE_CAPACITY, state.instances);
    ...
    // 相机固定在初始位置，安卓端没有键盘输入
    state.camera.position = glm::vec3(0.0f, 0.0f, 6.0f);
    ...
    state.camera.farPlane = 200.0f;
    state.camera.moveSpeed = 6.0f;
    ...
    LOG_I("android early z renderer initialized, instances %d", state.uiState.activeInstanceCount);
}
...
static void drawOneFrame(AppState& state)
{
    ...
    input.usePrepass = state.uiState.usePrepass;
    input.insertDiscard = state.uiState.insertDiscard;
    input.alphaTest = state.uiState.alphaTest;
    input.fragmentCostIterations = state.uiState.fragmentCostIterations;
    ...
    if (currentTime - state.lastPrintSeconds >= 2.0) {
        ...
        LOG_I("frame %.1f FPS, order %s, prepass %d, discard %d, alpha test %d", fps,
              orderName(state.uiState.orderMode), state.uiState.usePrepass ? 1 : 0,
              state.uiState.insertDiscard ? 1 : 0, state.uiState.alphaTest ? 1 : 0);
        state.lastPrintSeconds = currentTime;
    }
}

// TCP 控制命令：instances/order/prepass/discard/alpha-test/fragment-cost 改配置，
...
static void installControlHandler(AppState& state)
{
    state.controlServer.onCommand = [&state](const std::string& command) -> std::string {
        ...
        if (verb == "order") {
            ...
            state.uiState.orderMode = value == "back" ? DRAW_ORDER_BACK_TO_FRONT
        ...
        if (verb == "prepass" || verb == "discard" || verb == "alpha-test") {
            ...
            if (verb == "prepass") {
                state.uiState.usePrepass = enabled;
            } else if (verb == "discard") {
                state.uiState.insertDiscard = enabled;
            } else {
                state.uiState.alphaTest = enabled;
            }
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "fragment-cost") {
            ...
            state.uiState.fragmentCostIterations = static_cast<float>(value);
            resetTimingWindows(state.timingStore);
            return "ok";
        }
        if (verb == "begin") {
...
{
    AppState state;
    state.uiState.activeInstanceCount = 48;
    state.uiState.orderMode = DRAW_ORDER_FRONT_TO_BACK;
    state.uiState.usePrepass = false;
    state.uiState.insertDiscard = false;
    state.uiState.alphaTest = true;
    state.uiState.fragmentCostIterations = 64.0f;
