...
// 实例总数上限与默认值
static const uint32_t INSTANCE_CAPACITY = 256;

// 报告里 case 自己的前几列
static const char* const REPORT_HEADER_COLUMNS =
    "order,prepass,discard,alpha_test,fragment_cost,draw_commands";
...
{
    char prefix[160];
    std::snprintf(prefix, sizeof(prefix), "%s,%d,%d,%d,%.0f,%u", orderName(state.orderMode),
                  state.usePrepass ? 1 : 0, state.insertDiscard ? 1 : 0, state.alphaTest ? 1 : 0,
                  static_cast<double>(state.fragmentCostIterations), drawCommands);
    return std::string(prefix);
}

// 交换链以及所有跟它的尺寸、图像数量绑定的资源都要在窗口尺寸变化后重建
static void rebuildSwapchainResources(VulkanContext& ctx, EarlyZRenderer& renderer, GpuBuffer& captureBuffer,
                                      bool capturePending)
{
                             ...
                             EarlyZSceneUniform& outUniform)
{
    ...
    outUniform.frameParams = glm::vec4(state.fragmentCostIterations, state.insertDiscard ? 1.0f : 0.0f,
                                       state.alphaTest ? 1.0f : 0.0f, 0.0f);
}
...
int main(int argc, char** argv)
{
    ...
    uint32_t instanceCount = 48;
    uint32_t orderMode = DRAW_ORDER_FRONT_TO_BACK;
    bool usePrepass = false;
    bool insertDiscard = false;
    bool alphaTest = true;
    float fragmentCostIterations = 64.0f;
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--instances") == 0 && i + 1 < argc) {
            ...
            if (std::strcmp(value, "front") == 0) {
                orderMode = DRAW_ORDER_FRONT_TO_BACK;
            } else if (std::strcmp(value, "back") == 0) {
                orderMode = DRAW_ORDER_BACK_TO_FRONT;
            } else if (std::strcmp(value, "random") == 0) {
                orderMode = DRAW_ORDER_RANDOM;
            } else {
                FATAL("--order takes front, back or random");
            }
            ++i;
        } else if (std::strcmp(argv[i], "--prepass") == 0) {
            usePrepass = true;
        } else if (std::strcmp(argv[i], "--discard") == 0) {
            insertDiscard = true;
        } else if (std::strcmp(argv[i], "--no-alpha-test") == 0) {
            alphaTest = false;
        } else if (std::strcmp(argv[i], "--fragment-cost") == 0 && i + 1 < argc) {
            fragmentCostIterations = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
    ...
    if (fragmentCostIterations < 1.0f || fragmentCostIterations > 256.0f) {
        FATAL("--fragment-cost must be between 1 and 256");
    }
    ...
    GLFWwindow* window = glfwCreateWindow(1600, 900, "Vulkan early-Z Demo", nullptr, nullptr);
    if (window == nullptr) {
    ...
    buildOverlappingQuadInstances(INSTANCE_CAPACITY, instances);

    EarlyZRenderer renderer = {};
    createRenderer(ctx, renderer, quadMesh, instances);
    ...
    uiState.usePrepass = usePrepass;
    uiState.insertDiscard = insertDiscard;
    uiState.alphaTest = alphaTest;
    uiState.fragmentCostIterations = fragmentCostIterations;
    ...
    controlServer.onCommand = [&](const std::string& command) -> std::string {
        ...
        if (verb == "order") {
            std::string value;
            if (!(stream >> value) || (value != "front" && value != "back" && value != "random")) {
                return "err: order takes front, back or random";
            }
            uiState.orderMode = value == "back" ? DRAW_ORDER_BACK_TO_FRONT
                               : value == "random" ? DRAW_ORDER_RANDOM
                                                   : DRAW_ORDER_FRONT_TO_BACK;
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "prepass" || verb == "discard" || verb == "alpha-test") {
            long value = 0;
            ...
            if (verb == "prepass") {
                uiState.usePrepass = enabled;
            } else if (verb == "discard") {
                uiState.insertDiscard = enabled;
            } else {
                uiState.alphaTest = enabled;
            }
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "fragment-cost") {
            double value = 0.0;
            if (!(stream >> value) || value < 1.0 || value > 256.0) {
                return "err: fragment-cost out of range";
            }
            uiState.fragmentCostIterations = static_cast<float>(value);
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "begin") {
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        EarlyZSceneUniform sceneUniform;
        ...
        input.usePrepass = uiState.usePrepass;
        input.insertDiscard = uiState.insertDiscard;
        input.alphaTest = uiState.alphaTest;
        input.fragmentCostIterations = uiState.fragmentCostIterations;
        ...
        if (currentTime - lastPrintTime >= 0.5) {
            std::printf("order %s | prepass %s | discard %s | alpha test %s | cost %.0f | draw commands %u\n",
                        orderName(uiState.orderMode), uiState.usePrepass ? "on" : "off",
                        uiState.insertDiscard ? "on" : "off", uiState.alphaTest ? "on" : "off",
                        static_cast<double>(uiState.fragmentCostIterations), uiStatistics.drawCallCount);
