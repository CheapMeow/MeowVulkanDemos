#pragma once

#include "gpu_clock_lock.h"
#include "timing.h"

#include <cstdint>

// 界面上可以调整的参数
struct UiState {
    float cameraMoveSpeed;
    // 着色频率，取值见 renderer.h 的 ShadingFrequency
    uint32_t shadingFrequency;
    // 镜面模型，取值见 renderer.h 的 SpecularModel
    uint32_t specularModel;
    // 逐像素着色时的法线来源，取值见 renderer.h 的 NormalSource
    uint32_t normalSource;
    // 球的细分段数，改变时重建几何
    uint32_t segments;
    // 高光指数
    float shininess;
    // 平行光强度与环境项强度
    float lightIntensity;
    float ambient;
};

// 界面上显示的本帧工作量，耗时统计由 TimingStore 提供
struct UiStatistics {
    uint32_t drawCallCount;
    uint32_t triangleCount;
};

// 本 case 自己的界面文本，交给公共界面层构建字形范围并逐个校验字形
const char* const* caseInterfaceTexts(int& outCount);

// 构建本 case 的控制面板，公共的耗时面板与锁频面板由公共界面层绘制
void buildUserInterface(UiState& state, const UiStatistics& statistics, const TimingStore& timing,
                        GpuClockLockState& gpuClockLockState, GpuClockMonitor& gpuClockMonitor);
