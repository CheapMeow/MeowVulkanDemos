...
// 程序生成的细节贴图边长
enum { DETAIL_TEXTURE_SIZE = 1024 };

static void createRenderPass(const VulkanContext& ctx, LodRenderer& renderer)
{
...
static void createSwapchainTargets(const VulkanContext& ctx, LodRenderer& renderer)
{
...
static void destroySwapchainTargets(const VulkanContext& ctx, LodRenderer& renderer)
{
...
// 采样器按当前配置创建：是否使用多级纹理决定最大层级，另外加上偏置与各向异性
static void createDetailSampler(const VulkanContext& ctx, LodRenderer& renderer,
                                const LodSamplerOptions& options)
{
    ...
    // 关掉多级纹理时把最大层级钳到 0，无论屏幕上的导数多大都只取最粗的一级
    samplerInfo.maxLod = options.mipmapped ? static_cast<float>(renderer.detailTexture.mipLevels - 1) : 0.0f;
    samplerInfo.mipLodBias = options.lodBias;
    samplerInfo.anisotropyEnable = options.maxAnisotropy > 1.0f ? VK_TRUE : VK_FALSE;
    samplerInfo.maxAnisotropy = options.maxAnisotropy;
    VK_CHECK(vkCreateSampler(ctx.device, &samplerInfo, nullptr, &renderer.detailSampler));

    renderer.samplerOptions = options;
}

static void createDescriptorSetLayout(const VulkanContext& ctx, LodRenderer& renderer)
{
...
static void createDescriptorPool(const VulkanContext& ctx, LodRenderer& renderer)
{
...
static void createPipeline(const VulkanContext& ctx, LodRenderer& renderer)
{
    ...
    VkShaderModule vertexModule = loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/lod.vert.spv"));
    VkShaderModule fragmentModule =
        loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/lod.frag.spv"));
...
void createRenderer(const VulkanContext& ctx, LodRenderer& renderer, const MeshData& groundMesh)
{
    ...
    std::vector<unsigned char> detailPixels;
    buildDetailTexturePixels(DETAIL_TEXTURE_SIZE, detailPixels);
    createTextureFromRgba(ctx, DETAIL_TEXTURE_SIZE, DETAIL_TEXTURE_SIZE, detailPixels.data(), false,
                          renderer.detailTexture);
    std::printf("detail texture: %ux%u, mip levels %u\n", renderer.detailTexture.width,
                renderer.detailTexture.height, renderer.detailTexture.mipLevels);

    LodSamplerOptions defaultOptions = {};
    defaultOptions.mipmapped = true;
    defaultOptions.lodBias = 0.0f;
    defaultOptions.maxAnisotropy = 1.0f;
    createDetailSampler(ctx, renderer, defaultOptions);
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        LodFrameResources& frame = renderer.frames[i];
        ...
        createBuffer(ctx, sizeof(LodSceneUniform), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        ...
        writeImageDescriptor(ctx, frame.sceneSet, 1, renderer.detailTexture.view, renderer.detailSampler);
...
void destroyRenderer(const VulkanContext& ctx, LodRenderer& renderer)
{
    ...
    vkDestroySampler(ctx.device, renderer.detailSampler, nullptr);
    destroyTexture(ctx, renderer.detailTexture);
...
// 采样器配置改变时重建采样器并改写描述符
static void applySamplerOptions(const VulkanContext& ctx, LodRenderer& renderer, const LodSamplerOptions& options)
{
    const bool same = renderer.samplerOptions.mipmapped == options.mipmapped &&
                      renderer.samplerOptions.lodBias == options.lodBias &&
                      renderer.samplerOptions.maxAnisotropy == options.maxAnisotropy;
    if (same) {
        return;
    }

    // 旧的采样器可能还被另一帧使用，先等设备空闲再拆
    VK_CHECK(vkDeviceWaitIdle(ctx.device));
    vkDestroySampler(ctx.device, renderer.detailSampler, nullptr);
    createDetailSampler(ctx, renderer, options);
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        writeImageDescriptor(ctx, renderer.frames[i].sceneSet, 1, renderer.detailTexture.view,
                             renderer.detailSampler);
    }
}
...
{
    LodFrameResources& frame = renderer.frames[frameCounter % MAX_FRAMES_IN_FLIGHT];

    VK_CHECK(vkWaitForFences(ctx.device, 1, &frame.inFlight, VK_TRUE, UINT64_MAX));

    applySamplerOptions(ctx, renderer, input.sampler);
    ...
    std::memcpy(frame.sceneBuffer.mapped, &sceneUniform, sizeof(LodSceneUniform));
