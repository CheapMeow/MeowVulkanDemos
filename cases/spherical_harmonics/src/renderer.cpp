...
// 球的细分段数，辐照度本身是低频的，几何密度只影响轮廓
static const uint32_t SPHERE_SEGMENTS = 64;

const char* shBackgroundName(uint32_t background)
{
    return background == SH_BACKGROUND_ORIGINAL ? "original" : "reconstructed";
}

const char* shShadingName(uint32_t shading)
{
    return shading == SH_SHADING_REFERENCE ? "reference" : "coefficients";
}

static void createRenderPass(const VulkanContext& ctx, ShRenderer& renderer)
{
...
static void createSwapchainTargets(const VulkanContext& ctx, ShRenderer& renderer)
{
...
static void destroySwapchainTargets(const VulkanContext& ctx, ShRenderer& renderer)
{
...
static void createDescriptorLayout(const VulkanContext& ctx, ShRenderer& renderer)
{
...
static void createPipelines(const VulkanContext& ctx, ShRenderer& renderer)
{
...
void createRenderer(const VulkanContext& ctx, ShRenderer& renderer)
{
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        ShFrameResources& frame = renderer.frames[i];
        ...
        createBuffer(ctx, sizeof(ShUniform), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
...
{
    ShFrameResources& frame = renderer.frames[frameCounter % MAX_FRAMES_IN_FLIGHT];
    ...
    std::memcpy(frame.uniformBuffer.mapped, &uniform, sizeof(ShUniform));
