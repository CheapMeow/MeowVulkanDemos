...
static bool parseBackground(const std::string& value, uint32_t& background)
{
    if (value == "reconstructed") {
        background = SH_BACKGROUND_RECONSTRUCTED;
    } else if (value == "original") {
        background = SH_BACKGROUND_ORIGINAL;
    } else {
        return false;
    }
    return true;
...
static bool parseShading(const std::string& value, uint32_t& shading)
{
    if (value == "coefficients") {
        shading = SH_SHADING_COEFFICIENTS;
    } else if (value == "reference") {
        shading = SH_SHADING_REFERENCE;
    } else {
        return false;
    }
    return true;
...
static void rebuildSwapchainResources(VulkanContext& ctx, ShRenderer& renderer, GpuBuffer& captureBuffer,
                                      bool capturePending)
{
...
// 系数在 uniform 里按通道分别打包成 vec4 数组，每个通道占用七个
                        ...
                        const float* coefficientsRgb, ShUniform& outUniform)
{
    ...
    const EnvironmentParams defaults = defaultEnvironmentParams();
    outUniform.sunDirection = glm::vec4(defaults.sunDirection, 0.0f);
...
int main(int argc, char** argv)
{
    ...
    uiState.bandCount = 5;
    uiState.background = SH_BACKGROUND_ORIGINAL;
    uiState.shading = SH_SHADING_COEFFICIENTS;
    uiState.referenceSamples = 1024;
    uiState.rotationDegrees = 0.0f;
    uiState.exposure = 1.0f;
    uiState.skyIntensity = 1.0f;
    uiState.glowIntensity = 4.0f;
    uiState.glowExponent = 4.0f;
    uiState.projectionMilliseconds = 0.0;
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--auto-exit") == 0 && i + 1 < argc) {
        ...
        } else if (std::strcmp(argv[i], "--bands") == 0 && i + 1 < argc) {
            const long value = std::atol(argv[i + 1]);
            if (value < 1 || value > SH_MAX_BANDS) {
                FATAL("--bands takes an integer between 1 and %d", static_cast<int>(SH_MAX_BANDS));
            }
            uiState.bandCount = static_cast<uint32_t>(value);
            ++i;
        } else if (std::strcmp(argv[i], "--background") == 0 && i + 1 < argc) {
            if (!parseBackground(argv[i + 1], uiState.background)) {
                FATAL("--background takes reconstructed or original");
            }
            ++i;
        } else if (std::strcmp(argv[i], "--shading") == 0 && i + 1 < argc) {
            if (!parseShading(argv[i + 1], uiState.shading)) {
                FATAL("--shading takes coefficients or reference");
            }
            ++i;
        } else if (std::strcmp(argv[i], "--samples") == 0 && i + 1 < argc) {
            uiState.referenceSamples = static_cast<uint32_t>(std::atoi(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--rotation") == 0 && i + 1 < argc) {
            uiState.rotationDegrees = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
            ...
            uiState.glowIntensity = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
        } else if (std::strcmp(argv[i], "--exponent") == 0 && i + 1 < argc) {
            uiState.glowExponent = static_cast<float>(std::atof(argv[i + 1]));
            ++i;
    ...
    GLFWwindow* window = glfwCreateWindow(1600, 900, "Vulkan Spherical Harmonics Demo", nullptr, nullptr);
    if (window == nullptr) {
    ...
    Camera camera = {};
    camera.position = glm::vec3(0.0f, 0.0f, 3.6f);
    camera.yaw = 0.0f;
    camera.pitch = 0.0f;
    camera.verticalFieldOfView = glm::radians(55.0f);
    camera.nearPlane = 0.1f;
    camera.farPlane = 100.0f;
    camera.moveSpeed = uiState.cameraMoveSpeed;

    // 环境投影：球面上等距采样做数值积分，天空亮度或太阳亮度变化时才重算
    EnvironmentParams environment = defaultEnvironmentParams();
    environment.skyIntensity = uiState.skyIntensity;
    environment.glowIntensity = uiState.glowIntensity;
    environment.glowExponent = uiState.glowExponent;
    ...
    controlServer.onCommand = [&](const std::string& command) -> std::string {
        ...
        if (verb == "background") {
            std::string value;
            if (!(stream >> value) || !parseBackground(value, uiState.background)) {
        ...
        if (verb == "shading") {
            std::string value;
            if (!(stream >> value) || !parseShading(value, uiState.shading)) {
        ...
        if (verb == "samples") {
            ...
            uiState.referenceSamples = static_cast<uint32_t>(value);
            resetTimingWindows(timingStore);
            return "ok";
        }
        if (verb == "rotation" || verb == "exposure" || verb == "sky" || verb == "glow" ||
            ...
            if (verb == "rotation") {
                uiState.rotationDegrees = value;
            } else if (verb == "exposure") {
                uiState.exposure = value;
            } else if (verb == "sky") {
                uiState.skyIntensity = value;
                lastProjectedSky = -1.0f;
            } else if (verb == "glow") {
                uiState.glowIntensity = value;
                lastProjectedGlow = -1.0f;
            } else {
                uiState.glowExponent = value;
                lastProjectedExponent = -1.0f;
            }
            resetTimingWindows(timingStore);
            return "ok";
        }
    ...
    while (glfwWindowShouldClose(window) == 0) {
        ...
        if (uiState.skyIntensity != lastProjectedSky || uiState.glowIntensity != lastProjectedGlow ||
            uiState.glowExponent != lastProjectedExponent) {
            environment.skyIntensity = uiState.skyIntensity;
            environment.glowIntensity = uiState.glowIntensity;
            environment.glowExponent = uiState.glowExponent;
            uiState.projectionMilliseconds =
                projectEnvironment(environment, baseCoefficients) * 1000.0;
            lastProjectedSky = uiState.skyIntensity;
            lastProjectedGlow = uiState.glowIntensity;
            lastProjectedExponent = uiState.glowExponent;
            std::printf("projected the environment into %d spherical harmonics coefficients in %.1f ms\n",
                        static_cast<int>(SH_MAX_COEFFICIENTS), uiState.projectionMilliseconds);
        }

        // 旋转只作用在系数上，每帧都从原始系数重新旋转一份，避免误差累积
        std::memcpy(rotatedCoefficients, baseCoefficients, sizeof(rotatedCoefficients));
        rotateShAboutY(rotatedCoefficients, SH_MAX_BANDS, glm::radians(uiState.rotationDegrees));
        ...
        ShUniform uniform;
        fillUniform(camera, aspectRatio, uiState, rotatedCoefficients, uniform);
        ...
        if (currentTime - lastPrintTime >= 0.5) {
            std::printf("bands %u | background %s | shading %s | samples %u | rotation %.0f | draw commands %u\n",
                        uiState.bandCount, shBackgroundName(uiState.background),
                        shShadingName(uiState.shading), uiState.referenceSamples,
                        uiState.rotationDegrees, uiStatistics.drawCallCount);
