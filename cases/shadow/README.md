...
阴影通道只写入物体背面的深度，受光表面在深度比较时稳定处在已写入深度之前，从源头上减少自阴影条纹。

光栅化阶段再开启深度偏移，常数项 1.5、斜率项 2.0，抵消光栅化与深度量化的误差。

着色器一侧沿世界法线抬升采样点，并按入射角放大深度偏移：掠射角下深度误差更大，偏移量随之增大。
...
桌面端用 `--control-port 21000` 启动后，连接并逐行发命令：`instances`/`yaw`/`pitch` 改配置，`shadows 1`/`shadows 0` 开关阴影，`pcf 1`/`pcf 0` 开关软阴影，`shadow-size` 与 `shadow-bits` 改贴图配置并触发重建，`begin` 与 `end` 圈定一段测量（`end` 返回一行与 CSV 同格式的数据），`quit` 退出。
