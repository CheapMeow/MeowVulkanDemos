...
安卓端用同一套 TCP 分段测量在真机上扫描实例总量：200、1000、2000、10000、20000、100000。测试机为 Xiaomi 2203121C（Xiaomi 12，Adreno 730），显示 override 分辨率 1080x2400，应用横屏渲染；测量保留界面、帧率上限 60、GPU 锁频 364MHz。这台机器支持时间戳查询，设备时间为实测值。
...
200 实例一档的数据集中有两次可见数相同的重复，上表取两次平均（两次帧时间相差约 1~1.5 毫秒）。两行读数明显偏离同组其他路径：2000/478 与 20000/527 的实例化行帧时间都高于同组逐实例行，而两条路径调用的是同一个主机剔除函数，对应行的主机剔除也同步偏高（20000/527 一行实例化 8.3 毫秒对逐实例 2.2 毫秒），判为该段测量混入了手机运行波动，趋势判读不采用这两行。

**先定噪声标尺。** 手机端缺少重复到能统计同一工作项的多遍数据，直接用单段标准差判断：逐实例与实例化两行的帧时间标准差多处达到 5~11 毫秒，主机剔除等短时项的标准差可与均值同量级。因此下面只把跨多个配置稳定复现、且差值大于本行标准差的差距当作真实差异，小于几毫秒且不跨配置复现的差距一律按噪声处理。

**实例总量小的时候，indirect 相对实例化没有可测收益。** 总量 200、1000、2000 的三档里，indirect 与实例化的帧时间差分别是 0.2、0.0、0.2~1.6 毫秒，全部落在这几行的帧时间标准差（最高约 11 毫秒）之内；1000 实例那档两条路径都是 58.2 毫秒，读数几乎相同。这些配置的主机剔除只有零点几到一两毫秒，即使被 indirect 归零也淹没在帧间抖动里。

**indirect 的收益只随实例总量增长，与可见实例数量基本无关。** 主机剔除的成本由实例总量决定：同批测量里 1 万实例的逐实例与实例化行约 7.5~7.9 毫秒，10 万实例涨到 22.8~23.1 毫秒；indirect 行始终把它降到零，只多出 0.14~0.17 毫秒的剔除调度。因此同一可见负载（527 个可见实例）下，indirect 帧时间不随总量上升——1 万、2 万、10 万实例分别是 87.7、87.9、87.9 毫秒——而带主机剔除的两条路径从约 95 毫秒涨到 114 毫秒。

**两档收益各自对应一个规模量，与 PC 的结论一致。** 实例化省掉的是逐可见实例下发绘制命令的开销，只跟可见实例数量有关；indirect 在此基础上省掉的是在主机上遍历全部实例的开销，只跟实例总量有关。手机上的净效果：1 万实例时 indirect 比实例化快约 7 毫秒，10 万实例快约 26 毫秒（帧时间从 114.0 降到 87.9，省约两成三）；实例总量到几千以内时这条收益还抵不过测量噪声。场景里可见实例多、实例总量小，用实例化即可；只有实例总量大到主机剔除占明显份额时，indirect 才带来能测到的收益。
