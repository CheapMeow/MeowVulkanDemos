...
{
    if (mode == FILTER_BILINEAR) {
        return "bilinear";
    }
    if (mode == FILTER_BSPLINE) {
        return "bspline";
    }
    if (mode == FILTER_CATMULL_ROM) {
        return "catmull_rom";
    }
    if (mode == FILTER_LANCZOS2) {
        return "lanczos2";
    }
    if (mode == FILTER_LANCZOS3) {
        return "lanczos3";
    }
    if (mode == FILTER_ANALYTIC) {
        return "analytic";
    }
    return "nearest";
}
...
{
    if (mode == FILTER_NEAREST) {
        return 1;
    }
    if (mode == FILTER_BILINEAR) {
        return 4;
    }
    if (mode == FILTER_BSPLINE || mode == FILTER_CATMULL_ROM) {
        return 16;
    }
    if (mode == FILTER_LANCZOS2) {
        return 16;
    ...
    return referenceSide * referenceSide;
}

static void createRenderPass(const VulkanContext& ctx, FilterRenderer& renderer)
{
...
static void createSwapchainTargets(const VulkanContext& ctx, FilterRenderer& renderer)
{
...
static void destroySwapchainTargets(const VulkanContext& ctx, FilterRenderer& renderer)
{
...
static void createDescriptorLayout(const VulkanContext& ctx, FilterRenderer& renderer)
{
...
static void createPipeline(const VulkanContext& ctx, FilterRenderer& renderer)
{
    VkPipelineLayoutCreateInfo layoutInfo = {};
    layoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    layoutInfo.setLayoutCount = 1;
    layoutInfo.pSetLayouts = &renderer.sampleSetLayout;
    ...
    VkShaderModule fragmentModule =
        loadShaderModuleFromMemory(ctx, readAssetBytes(DEMO_SHADER_DIR "/sample.frag.spv"));
...
void createRenderer(const VulkanContext& ctx, FilterRenderer& renderer)
{
    ...
    const std::vector<unsigned char> pixels = buildPatternPixels();
    createTextureFromRgba(ctx, PATTERN_TEXTURE_SIZE, PATTERN_TEXTURE_SIZE, pixels.data(), false,
                          renderer.patternTexture, nullptr);

    // 采样全部用 texelFetch 手工完成，采样器只负责寻址
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i) {
        ...
        createBuffer(ctx, sizeof(SampleUniform), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                     frame.uniformBuffer);

        frame.sampleSet = allocateDescriptorSet(ctx, renderer.descriptorPool, renderer.sampleSetLayout);
        writeBufferDescriptor(ctx, frame.sampleSet, 0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                              frame.uniformBuffer);
        writeImageDescriptor(ctx, frame.sampleSet, 1, renderer.patternTexture.view,
                             renderer.patternSampler);
...
void destroyRenderer(const VulkanContext& ctx, FilterRenderer& renderer)
{
    ...
    vkDestroyPipeline(ctx.device, renderer.samplePipeline, nullptr);
    ...
    vkDestroyDescriptorSetLayout(ctx.device, renderer.sampleSetLayout, nullptr);
               ...
               const FrameInput& input, const SampleUniform& uniform, FrameStatistics& outStatistics)
{
    ...
    std::memcpy(frame.uniformBuffer.mapped, &uniform, sizeof(SampleUniform));
    ...
    const double sampleStart = nowSeconds();
    ...
    vkCmdBindDescriptorSets(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer.pipelineLayout, 0, 1,
                            &frame.sampleSet, 0, nullptr);
    vkCmdBindPipeline(frame.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderer.samplePipeline);
    vkCmdDraw(frame.commandBuffer, 3, 1, 0, 0);

    outStatistics.drawCallCount = 1;
    outStatistics.renderPassCount = 1;
    outStatistics.cpuRecordSampleMilliseconds = (nowSeconds() - sampleStart) * 1000.0;
