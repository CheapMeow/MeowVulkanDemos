...
纹理是一段频率沿半径线性增长的图案（两个错开的频率渐变叠加，三个通道用不同相位），整张图 512 乘 512。全屏通道把它按缩放系数采样一遍：缩放小于 1 时纹理被放大，能看到重建核的形状；大于 1 时纹理被缩小，单个像素盖住多个纹素。参考一档不走纹理，直接按解析公式算图案，放大时取像素中心，缩小时按像素覆盖的区域做超采样，相当于理想的重建。
...
$f_i$ 是纹素值，$k$ 是重建核，$x$ 是采样位置。核的性质决定画面的样子：支撑越宽、越接近理想的 sinc，细节保留得越多，代价是过冲与读取次数。

五种核的权重（$t$ 是采样点到纹素中心的距离）：
...
两个方向的核是同一套权重相乘，因此一次采样要读 $4$、$16$ 或 $36$ 个纹素。Lanczos 的核在零点要单独处理，否则 $\sin(0)/0$ 会算出非数。
...
按误差排序是 Catmull-Rom 最好，Lanczos3 与双线性其次，三阶 B 样条最差。两个插值核（Catmull-Rom 与 Lanczos）在整数纹素位置上的权重正好是 $\delta$ 函数，重建能精确通过原来的样本；三阶 B 样条是拟合核，它在整数位置上的权重是 $\frac{1}{6}, \frac{4}{6}, \frac{1}{6}$，等于自己又做了一次平滑，所以偏软。最近邻的误差比其余所有核大一个数量级，画面是一块块方格。
...
频率一高，核之间的差距立刻拉开：Catmull-Rom 与三阶 B 样条差八倍（0.3121 对 2.5053）。超过 8 的像素占比那一列反而要小心看：Catmull-Rom 是 0.40%、三阶 B 样条是 5.27%，但两者的最大差值都是几十个灰阶——那是插值核在少数位置上的过冲，画面上一圈很窄的亮边或暗边。

### 缩小八倍：换核解决不了

缩放 8 时纹理在画面里平铺八份，每个屏幕像素盖住 64 个纹素。此时正确的做法是对像素覆盖的面积做积分，而上面那些核都只读 1 到 36 个纹素，离一次平均还差得远：
...
六种核的平均绝对差都在 25 到 27 之间，彼此差不到 10%。参考图的平均拉普拉斯响应只有 50.00，而这些核给出的画面在 128 到 137 之间，是正确值的两倍半以上：多出来的全部是摩尔纹。三阶 B 样条略微最好（24.66），因为它的 16 个权重全为正，最接近一次平均；Lanczos3 反而最差，它的负旁瓣把中心样本的权重抬得更高，等于在放大高频而不是压掉它。

结论是缩小时换更贵的核没有用：缺的是把 64 个纹素先平均掉这一步，只能靠多级渐远纹理或其它预过滤手段。这也解释了为什么各向异性过滤要沿长轴取多个样本再合并，而不是把双线性换成三次插值。

### 参考本身也要收敛

参考路径按超采样近似像素覆盖面积上的积分。以 16 乘 16 为基准，其余档位与它的差：
...
只取中心就是点采样，与真正的面积平均差 27 个灰阶——这正是上面那张表里各核的误差量级，说明那些误差几乎全部来自欠采样，而不是核的形状。四乘四（每像素 16 个采样）之后误差降到 0.95，八乘八降到 0.28。
...
读取次数相同的两个立方核代价不同：Catmull-Rom 与三阶 B 样条都是十六次读取，前者 0.035、后者 0.041，差在权重公式里的运算量。Lanczos2 同样是十六次读取却要 0.071，因为它每个权重都要算两次正弦；Lanczos3 的三十六次读取加上正弦，代价是双线性的十六倍。参考的十六乘十六要算 256 次图案函数，0.485 毫秒，比 Lanczos3 贵三倍多——用它来衡量别的核只适合离线跑。
