...
场景里三块大的遮挡物挡在前面，后面散布两万个小方块。遮挡剔除在计算着色器里完成：每个实例的包围盒投影到
屏幕，取出覆盖到的整片金字塔纹素，与包围盒的最近深度比较，可见实例写进一个列表，间接绘制命令的实例数与
索引数由一次有序压缩写出来。
...
金字塔每一级的纹素存的是它覆盖范围内深度的极值。取最远深度时，包围盒的最近点比这一片纹素里最远的场景
深度还远，说明整个包围盒确实在场景几何之后，剔除不会漏掉可见物体；取最近深度时，只要包围盒覆盖到的任意
一个纹素上有近处物体，整个包围盒就会被剔掉，跨在遮挡物边缘上的方块因此整块消失。
...
可见实例是 2353，固定 0 级是 925。按包围盒屏幕投影尺寸选层级时，场景里的方块都落在 0 级上，结果与固定
0 级一致；这条公式的意义在于让一次测试读取的纹素数量与包围盒覆盖的像素数无关，方块贴近相机时测试开销
不会跟着涨。

包围盒扩大系数把测试用的包围盒放大，跨在遮挡物边缘上的方块更容易保住，代价是漏剔变多：系数从 1.0 调到
1.6，可见实例从 925 升到 1506。

### 深度来源

深度取自上一帧时不需要预通道，剔除在帧首就能给出结果，代价是画面滞后一帧；深度取自当前帧时先跑一遍
深度预通道，得到的遮挡关系与当前画面完全一致，代价是全部实例都要过一遍深度。两条路径都只影响通道的
组织方式，判定使用的深度与颜色通道写出的深度在两条路径下相同，抓帧结果也完全相同。

### 有序压缩

剔除通道只把可见标志写到标志缓冲，可见列表与间接绘制命令由一次连续前缀和写出，顺序按实例编号排列。
用原子加做压缩时写出顺序随线程组的调度变化，场景里有大量小方块彼此重叠，深度在浮点意义下完全相同的像素
上，绘制顺序一变颜色就跟着变，画面会出现少量随运行变化的像素。改成按编号顺序写出以后，可见列表的顺序
完全确定，抓帧结果可以逐像素比较，取最远深度的保守判定也因此能被验证到像素级。
...
0 级（全分辨率的八分之一）有 20.5% 的像素落在几何上，1 级（六十四分之一）只剩 15.4%：级别越粗，纹素的
极值被背景顶掉的比例越高，这张图直接解释了为什么 1 级的剔除数量比 0 级少。
